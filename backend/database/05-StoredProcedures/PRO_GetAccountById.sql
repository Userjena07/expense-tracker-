...
/*
EXEC PRO_GetAccountById
    @Id           = 1,
    @RequestedBy  = 1,
    @SessionToken = 'jwt-session-guid';
*/
CREATE OR ALTER PROCEDURE [dbo].[PRO_GetAccountById]
    @Id           BIGINT       = 0,
...
BEGIN
    ...
    SELECT 1 AS [Status], 'ACCOUNT_FOUND' AS [MessageCode];
    ...
    FROM [dbo].[Account] a
    WHERE a.[Id] = @Id AND a.[UserId] = @RequestedBy AND a.[IsDeleted] = 0;
END
GO
